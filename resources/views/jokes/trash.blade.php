<x-admin-layout>
    ...
    <section class="py-6 mx-12 space-y-4">
            ...
            <div class="flex gap-4 justify-end">
                ...
                <form method="post" action="{{ route('admin.jokes.trash.remove.all') }}">
                    ...
                    <x-secondary-button class="overflow-hidden justify-center" type="submit">
                        <i class="fa-solid fa-delete-left text-lg pr-2"></i> {{ __('Remove All') }}
                    </x-secondary-button>
                </form>
            </div>
        ...
        <table class="table w-full">
            ...
            <tr>
                <td class="w-2/3 p-2 rounded-tl-lg">{{ __('Title') }}</td>
                <td class="p-2 pr-8 text-right">{{ __('Author') }}</td>
                <td class="w-1/6 p-2 rounded-tr-lg">{{ __('Actions') }}</td>
            </tr>
            </thead>
            <tbody>
                    ...
                    <td class="p-2 flex gap-4">
                        <form method="post" action="{{ route('admin.jokes.trash.recover.one', $joke) }}">
                            ...
                            <x-primary-button class="overflow-hidden justify-center" type="submit">
                                ...
                                <span class="sr-only">{{ __('Recover') }}</span>
                            </x-primary-button>
                        </form>
            ...
            @empty
                <tr>
                    <td colspan="3" class="p-2 pt-4">{{ __('No items in the trash') }}</td>
                </tr>
            @endforelse
            </tbody>
            <tfoot>
            <tr>
                <td colspan="3" class="pt-4">{{ $jokes->links() }}</td>
            </tr>
            </tfoot>
        </table>
    </section>
